...
Usage:
    python scripts/generate_trainer.py [options]
    
Examples:
    # Basic decoder-only training
    python scripts/generate_trainer.py --model decoder-only --steps 1000
    
    # Encoder-decoder with custom image size
    python scripts/generate_trainer.py --model encoder-decoder --image-size 512 --batch-size 4
    
    # Inference mode
    python scripts/generate_trainer.py --inference --model decoder-only --resume-ckpt latest.pt
"""
...
# Generated by generate_trainer.py on {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
# Model: {args.model}
