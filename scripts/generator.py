...
class Generator:
    ...
    def _build_overrides(self, args, output_dir):
        ...
        if args.test_every is not None:
            overrides.append(f'training.test_every={args.test_every}')
        if args.test_1enc1ss is not None and args.test_1enc1ss:
            overrides.append('training.test_1enc1ss=true')
        if args.test_2enc2ss is not None and args.test_2enc2ss:
            overrides.append('training.test_2enc2ss=true')
        if args.test_layers is not None:
            # To avoid YAML parser errors, convert list/tuple/str to a comma-separated string
            if isinstance(args.test_layers, (list, tuple)):
                overrides.append(f'training.test_layers=[{",".join(str(x) for x in args.test_layers)}]')
            else:
                overrides.append(f'training.test_layers={args.test_layers}')
        if args.test_batch_size is not None:
            overrides.append(f'training.test_batch_size_per_gpu={args.test_batch_size}')
        return overrides
...
#SBATCH --gpus-per-node={slurm['gpu_type']}:{slurm['gpu_count']}
...
echo "GPUs: {slurm['gpu_count']}x {slurm['gpu_type']}"
...
def main():
    ...
    parser.add_argument('--test-1enc1ss', action='store_true', default=None,
                        help='Test with 1 encoder and 1 ss view setting')
    parser.add_argument('--test-2enc2ss', action='store_true', default=None,
                        help='Test with 2 encoder and 2 ss view setting')
    parser.add_argument('--test-layers', type=int, nargs='+',
                        help='List of layers to test (space-separated)')
    parser.add_argument('--test-batch-size', type=int,
                        help='Test batch size per GPU')
    ...
    parser.add_argument('--gpu-type', choices=['l40s', 'h100'],
                        help='GPU type')
